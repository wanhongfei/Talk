...
void CTcDlg::OnButtonConn() 
{
	...
	SendMsg(m_strCName, CString("all"), CString("进入聊天室！"), FALSE);
...
//实际执行发送的函数
void CTcDlg::SendMsg(CString &m_local_name, CString &m_dest_name, CString &m_strMsg, bool st)
{
	if (m_pArchiveOut!=NULL)
	{
		CMsg msg;  //创建一个消息对象
		//将要发送的信息文本赋给消息对象的成员变量
		msg.m_local_name = m_local_name;
...
////当单击“断开“按钮时，执行此函数，作客户退出聊天室的相关处理
void CTcDlg::OnButtonClose() 
{
	SendMsg(m_strCName, CString("all"), CString("离开聊天室！"), TRUE);
